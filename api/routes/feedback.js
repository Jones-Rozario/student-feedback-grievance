...
router.get("/faculty/avg/:facultyId", async (req, res) => {
  try {
    const { facultyId } = req.params;
    const feedbacks = await Feedback.find({ faculty: facultyId })
      .populate("course", "name code")
...
router.get("/faculty/ratings/:facultyId", async (req, res) => {
  try {
    const { facultyId } = req.params;
  ...
  try {
    ...
    Object.keys(courseGroups).forEach(courseId => {
      ...
      const totalRating = courseFeedbacks.reduce((sum, feedback) => {
        const avgRating = feedback.questionRating.reduce((qSum, q) => qSum + q.rating, 0) / feedback.questionRating.length;
...
router.get("/faculty/yearly/:facultyId", async (req, res) => {
  try {
    const { facultyId } = req.params;
    const feedbacks = await Feedback.find({ faculty: facultyId })
      .populate("course", "name code")
...
router.get("/check/:studentId/:courseId/:batch/:semester", async (req, res) => {
  try {
